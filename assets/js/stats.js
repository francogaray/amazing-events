...
function statsInitializer() {
    ...
    orderByPercentageAssistance.forEach((ev) => {
        ...
        if (!pastEventsArray.some((item) => ev.category == item.category)) {
            ...
            pastEventsArray.some((item) => ev.category == item.category)
        ) {
    ...
    orderByPercentageEstimate.forEach((ev) => {
        ...
        if (!upcomingEventsArray.some((item) => ev.category == item.category)) {
            ...
            upcomingEventsArray.some((item) => ev.category == item.category)
        ) {
    ...
    eventStats.innerHTML = eventsStatistics(
        orderByPercentageAssistance,
        orderByCapacity
    );
    ...
    function eventsStatistics(eventsAttendance, eventsCapacity) {
        ...
        for (let i = 0; i < 3; i++) {
            rows += `<tr>
                ...
                (eventsAttendance[i].assistance /
                    eventsAttendance[i].capacity) *
                100
            ).toFixed(2)} %</td >
                    <td>${
                ...
                (eventsAttendance[eventsAttendance.length - i - 1].assistance /
                    eventsAttendance[eventsAttendance.length - i - 1]
                        .capacity) *
                100
            ).toFixed(2)} %</td >
